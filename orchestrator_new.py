...
def main():
    ...
    emb_cfg = cfg.model['embeddings']
    w2v_model = train_word2vec_model(
        texts=X_train_combined,
        vector_size=emb_cfg['w2v_embedding_dim'],
        window=5,
        min_count=20,
        epochs=10,
        logger=logger,
    ...
    max_seq_len = emb_cfg['max_seq_length']
    X_train_seq = [
        text_to_sequence(
            text=text,
            vocab=vocab_w2v,
    ...
    model = UnfrozenCNNWithResiduals(
        ...
        cat_hidden_size=arch_cfg['cat_hidden_size'],
        emb_hidden_size=arch_cfg['emb_hidden_size'],
        num_filters=arch_cfg['num_filters'],
        dropout=arch_cfg['dropout_rate'],
    ).to(device)
    ...
    model, history, elapsed_time = train_model(
        ...
        optimizer_fn=train_cfg['optimizer'],
        loss_fn=train_cfg['loss_function'],
        device=device,
        ...
        scheduler_patience=train_cfg['scheduler']['patience'],
        scheduler_factor=train_cfg['scheduler']['factor'],
        logger=logger,
        multi_input=True,
    ...
    plot_losses_curves(
        history=history,
        loss_fn=train_cfg['loss_function'],
        plot_dir=os.path.join(cfg.paths['plots_dir'], f"{model_name}_training_curves.png"),
        logger=logger,
    )
